...
#       echo "set cluster_local_path `${LHAPDFCONFIG} --datadir`" >> mgconfigscript 
  ...
  #MODELSOURCE=https://cms-project-generators.web.cern.ch/cms-project-generators/ #default
  MODELSOURCE=https://amarini.web.cern.ch/amarini/ #2HDMtII_NLO.tar.gz
  ...
  if [ -e $CARDSDIR/${name}_extramodels.dat ]; then
    ...
    do
      ...
      if [[ $model = *[!\ ]* ]]; then
        ...
        wget --no-check-certificate $MODELSOURCE$model	
        cd models
        if [[ $model == *".zip"* ]]; then
          unzip ../$model
        elif [[ $model == *".tgz"* ]]; then
